...
linear_params = {'A': 0.04395604396, 'B': 15.20879121}
...
def __main__ ():
	...
	for frame in camera.capture_continuous (rawCapture, format ="bgr", use_video_port=True):
		...
		if phase == 0:
			...
			stickImg (mask[-1], screen_frame = 1)
			phase = 2
		elif phase == 2:
			mask.append (detectSpot(image) - bg_mask)
			stickImg (mask[-1], screen_frame = 1)
		...
		stickImg (bg_mask, screen_frame = 2)
		stickDistance ('Distance: ' + distance_txt + 'cm', update = True)
